...
- **NetworkPolicy**: Controlling network traffic between pods using policies
- **Namespace Selectors**: Selecting pods from specific namespaces
- **Ingress Rules**: Defining allowed incoming traffic to pods
- **Pod Selectors**: Selecting target pods using labels
- **Policy Evaluation**: Understanding how multiple policies combine (additive)
- **Network Security**: Implementing zero-trust network segmentation
- **Policy Analysis**: Reading and comparing different NetworkPolicy configurations
...
- [Namespace Selector](https://kubernetes.io/docs/concepts/services-networking/network-policies/#behavior-of-to-and-from-selectors)
