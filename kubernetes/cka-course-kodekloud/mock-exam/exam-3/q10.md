...
- **Deployment Scaling**: Using kubectl scale to change replica counts
- **Controller Manager**: Understanding the role of kube-controller-manager
- **ReplicaSet Controller**: How controller-manager manages replica counts
- **Control Plane Troubleshooting**: Diagnosing control plane component failures
- **Static Pod Manifests**: Understanding /etc/kubernetes/manifests directory
- **Pod Spec Errors**: Identifying and fixing typos in container commands
- **sed Command**: Using sed for find-and-replace in configuration files
- **kubectl get pods -n kube-system**: Checking control plane component health
